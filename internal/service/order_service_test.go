...
func setupOrderService(t *testing.T) *service.OrderService {
	cfg := &config.Config{
		DB: config.DBConfig{
			Host:     "localhost",
			Port:     5432,
...
func TestProcessOrder(t *testing.T) {
	orderService := setupOrderService(t)
	...
	err = orderService.ProcessOrderData(orderData)
	require.NoError(t, err)

	fetchedOrder, found := orderService.GetOrder("b563feb7b2b84b6test")
	assert.True(t, found)
	assert.Equal(t, order.OrderUID, fetchedOrder.OrderUID)
}

func TestProcessOrderWithMissingUID(t *testing.T) {
	orderService := setupOrderService(t)
